...
attn_other = mx.sym.var('attn_other', shape=(1, num_hidden))
...
def attention(method, enc_len, num_hidden, encoder_outputs, decoder_output):
    if method == 'general':
        left = decoder_output
        out = mx.sym.reshape(data=encoder_outputs, shape=(-3, 0))
        out = mx.sym.FullyConnected(data=out, weight=attn_fc_weight, 
                                            bias=attn_fc_bias, num_hidden=num_hidden,
                                            name='fc_decoder_output_%d' % i)
        right = mx.sym.reshape(data=out, shape=(batch_size, -1, num_hidden))

    elif method == 'dot':
        left = decoder_output
        right = encoder_outputs
    elif method == 'concat':
        out = mx.sym.expand_dims(data=decoder_output, axis=1)
        out = mx.sym.tile(data=out, reps=(1, enc_len, 1))
        out = mx.sym.concat(out, encoder_outputs, dim=2)
        out = mx.sym.reshape(data=out, shape=(-1, 2*num_hidden))
        out = mx.sym.FullyConnected(data=out, weight=attn_fc_weight, 
                                            bias=attn_fc_bias, num_hidden=num_hidden)
        right = mx.sym.reshape(data=out, shape=(batch_size, -1, num_hidden))
        left = mx.sym.tile(data=attn_other, reps=(batch_size, 1))
    else:
        assert False

    # (b, 1, n)  n is num_hidden
    left = mx.sym.expand_dims(data=left, axis=1, name='expand_left_%d' % i)
    # (b, 1, n) (b, seq, n) => (b, 1, seq)
    attn_weight = mx.sym.batch_dot(left, right, transpose_b=True, name='attn_weight_%d' % i)
...
def sym_gen(seq_len):
    ...
    for i in range(dec_len):
        ...
        attn_weight = attention('general', enc_len, num_hidden, encoder_outputs, decoder_output)
